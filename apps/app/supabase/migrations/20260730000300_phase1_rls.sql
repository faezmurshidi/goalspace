...
-- Child tables: flat ownership for reads/deletes, one shallow EXISTS for public
-- reads. The insert and update checks also require the (post-write) project_id
-- to belong to the caller, so ownership cannot be forged by pointing at, or
-- relocating a row into, someone else's project. USING stays flat on update
-- (it only gates which existing rows are reachable); WITH CHECK carries the
-- parent-project EXISTS because it validates the row as it will exist after
-- the write, which is exactly where a project_id reassignment would land.
--
...
create policy entries_insert on entries for insert
  with check (owner_id = auth.uid()
    and exists (select 1 from projects p where p.id = entries.project_id and p.owner_id = auth.uid()));
...
create policy work_items_insert on work_items for insert
  with check (owner_id = auth.uid()
    and exists (select 1 from projects p where p.id = work_items.project_id and p.owner_id = auth.uid()));
...
create policy documents_insert on documents for insert
  with check (owner_id = auth.uid()
    and exists (select 1 from projects p where p.id = documents.project_id and p.owner_id = auth.uid()));
...
create policy document_revisions_insert on document_revisions for insert
  with check (owner_id = auth.uid()
    and exists (select 1 from projects p where p.id = document_revisions.project_id and p.owner_id = auth.uid()));
...
create policy attachments_insert on attachments for insert
  with check (owner_id = auth.uid()
    and exists (select 1 from projects p where p.id = attachments.project_id and p.owner_id = auth.uid()));
