...
begin
  ...
  if v_proposal.kind = 'entry' then
    ...
    values (
      ...
      -- New siblings go last. Computed inside this transaction rather than by
      -- a separate read first, which is how the TypeScript version did it and
      -- is a lost update waiting to happen: two accepts racing both read the
      -- same maximum and both claim the next index.
